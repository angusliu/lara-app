...
<html lang="en-US" dir="ltr">
    <head>
        ...
        <title>Detect Element in Viewport</title>
    </head>
    <body style="font-size: 1.2rem">

<h1>孫子兵法(原文加白話翻譯)</h1>

<h2 id="topic-1">始計第一</h2>
...
6、從前殷商的興起，在於重用了在夏朝為臣的伊摯，他熟悉並瞭解夏朝的情況；周朝的興起，是由於周武王重用了瞭解商朝情況的呂牙。所以，明智的國君，賢能的將帥，能用智慧高超的人充當間諜，就一定能建樹大功。這是用兵的關鍵，整個軍隊都要依靠間諜提供的敵情來決定軍事行動。
    ...
    function onVisibilityChange(el, callback) {
        var old_visible;
        return function () {
            var visible = isElemInViewport(el);
    ...
    var handler = onVisibilityChange($('body'), function() {
        /* Your code go here */
    });

    var elem = $('#topic-1');
    var my_handler = function(evt) {
        var visible = isElemInViewport(elem);
        console.log(`${evt.type} '#${elem.attr('id')}' in viewport: ${visible}`);
    };

    $(window).on('DOMContentLoaded load resize scroll', my_handler);
    ...
    $(window).on('DOMContentLoaded load resize scroll', function() {
        var elem = $('#topic-3');
        var visible = isElemInViewport(elem);
        console.log(`'#${elem.attr('id')}' in viewport: ${visible}`);
    });
    //*/
